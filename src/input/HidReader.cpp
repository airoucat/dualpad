...
namespace
{
    ...
    hid_device* TryOpenDualSense()
    {
        ...
        while (cur) {
            if ((cur->vendor_id == dse::kSonyVid) &&
                ...
                if (dev) {
                    // Reader loop uses hid_read_timeout(); keep blocking mode so the
                    // dynamically computed timeout actually takes effect.
    ...
    void ReaderLoop()
    {
        ...
        while (g_running.load()) {
            ...
            const auto readTimeoutMs = static_cast<int>(
                dualpad::haptics::HidOutput::GetSingleton().GetReaderPollTimeoutMs(8));
            const int n = hid_read_timeout(dev, buf, sizeof(buf), readTimeoutMs);
