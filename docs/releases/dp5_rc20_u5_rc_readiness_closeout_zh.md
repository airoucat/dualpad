...
PR-A/B1/B2/B3 合入后，本仓库已有远端 `phase8` 与 `rc-readiness` 证据覆盖 RC gate、menu glyph reattach、upstream route health fail-closed 与 broken INI fail-closed。PR-C 只修正文档合同卫生、死链和状态滞后；PR-C 合入且最终 `main` head 再次通过远端 `phase8` 与 `rc-readiness` 后，新的 `main` merge commit 才可称为 `RC QA baseline`。
...
历史 GitHub #8 的 U2-U5 unchecked 小节已由 comment 标记为 superseded；最终 U2-U5 状态以 #13、本文与 `docs/authoritative-baseline/dp5_rc20_contract_zh.md` 为准。
