...
| `DualPadRuntime` mutation | `InputFramePump` 绑定的唯一 owner tick | debug/replay view | `RuntimeOwnerGuard` 拒绝重入、重复/倒退 token、thread drift 和 stop 后重绑 |
...
SKSE task fallback 必须经过同一个 `RuntimeOwnerGuard`。若它实际运行在不同线程，guard 会永久进入 degraded 并发布 neutral output；不会静默建立第二个 writer。当前 host tests 已证明 guard 状态机，真实 Skyrim 的线程 ID、cadence、重入和生命周期仍属于动态证据门。
...
待证明：Skyrim 1.5.97 实际 owner/Poll/UI thread ordering、hook gateway 运行、真实 pulse 可见性、长时间 soak 与 Favorites crash dump 闭环。在这些证据完成前，发布状态不得为 `GO`。
