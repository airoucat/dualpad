...
- RC20 hotfix Unit 9：host stress、canonical gates、文档、builder memory 与 graphify close-out 已完成。
- RC20 hotfix Unit 10：摇杆与 Journal L2/R2 matching safe smoke 已通过；matching Favorites dump、physical/synthetic DPadUp A/B、真实 Skyrim Favorites 循环和 soak 仍待完成。native `Game.Favorites` 保持 fail-closed，当前 release status 为 `GO WITH NATIVE FAVORITES DISABLED`。
