...
1. `manifestEpoch`
   - 来自 Phase 1 已发布的 `ActionManifest.manifestEpoch`
2. `contextRevision`
...
4. `deviceFamilyRevision`
   - 只能来自 `IngressEvent(kind = SourceEvidence)` 携带的
     `SourceEvidenceSnapshot.deviceFamilyEvidence.deviceFamilyRevision`；
...
4. `deviceFamilyRevision` 不允许从 `PublishedPresentationState.family` 逆推，也不允许直接读取
   ...
   `IngressEvent(kind = SourceEvidence)` 中的
   `SourceEvidenceSnapshot.deviceFamilyEvidence.deviceFamilyRevision`。
...
- Phase 7 只允许从 `IngressEvent(kind = SourceEvidence)` 携带的
  `SourceEvidenceSnapshot.deviceFamilyEvidence.deviceFamilyRevision` 读取边界所需 revision；
  `PublishedPresentationState.deviceFamilyRevision` 只视为下游镜像 / explain 字段，不得参与 boundary 判定。
- `FrameAssembler` 必须先把这一路径上的值原样拷入
  `AssembledFactFrame.facts.sourceEvidence.deviceFamilyEvidence.deviceFamilyRevision`，
  再同步填入 `IngressBoundaryKey.deviceFamilyRevision`；不允许在 assembler 内重新推断、
  合并或延迟到 projection/runtime 阶段再补值。
...
3. `docs/plans/dualpad_rearchitecture/04_slice_phase3_presentation_split_zh.md`
   - `DeviceFamilyIngressPublisher -> SourceEvidenceCollector` 已稳定发布
     `SourceEvidenceSnapshot.deviceFamilyEvidence.deviceFamilyRevision`
   - Phase 7 只允许消费这一路径；若仍只能从 `family`、`PublishedPresentationState.deviceFamilyRevision`
     或匿名 adapter 逆推，则不得进入本 slice。
...
以下补充合同用于收口本页仍残留的旧口径；若与上文较早段落冲突，以本节为准。
